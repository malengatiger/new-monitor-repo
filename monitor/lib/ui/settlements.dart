...
import 'package:monitor/monitor_bloc.dart';
...
class _SettlementListState extends State<SettlementList> {
  ...
  _getSettlements() async {
    ...
    if (country == null) {
      countries = await monBloc.getCountries();
      if (countries.length == 1) {
        country = countries.elementAt(0);
      }
    }
    if (country != null) {
      await monBloc.findSettlementsByCountry(country.countryId);
    }
    setState(() {
      isBusy = false;
    });
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: monBloc.settlementStream,
