...
public class RateLimitInterceptor implements HandlerInterceptor {
    ...
    public RateLimitInterceptor(Bucket bucket, int numTokens) {
        ...
        LOGGER.info(mm + "RateLimitInterceptor construction, tokens: " + numTokens);
    }
    ...
    public boolean preHandle(@NotNull HttpServletRequest request, @NotNull HttpServletResponse response,
        ...
        if (probe.isConsumed()) {
            LOGGER.info(mm + Emoji.OK + Emoji.OK + " RateLimitInterceptor: RemainingTokens: "
                    + probe.getRemainingTokens() + Emoji.YELLOW_BIRD + " Milliseconds to wait: " + TimeUnit.NANOSECONDS.toMillis(probe.getNanosToWaitForRefill()));
            response.addHeader("X-Rate-Limit-Remaining",
                    Long.toString(probe.getRemainingTokens()));
            return true;
        }

        LOGGER.info(mm + Emoji.ERROR + Emoji.NOT_OK +  "RateLimitInterceptor: RemainingTokens: "
